...
- **実害 A (ricdom): `applyTheme` が前テーマの inline 変数を消さない**。`light` / `dark` / `teal` に `--ric-popup-bg` / `--ric-popup-blur` / `--ric-panel-shadow` が無く、`applyTheme` は新パレットのキーを `setProperty` するだけ → cyber → dark で popup / panel の面だけ cyber のまま (統括がキー集合を機械的に比較: LIGHT/DARK/TEAL 24 キー、GLASS 26、CYBER/AQUA 29)。**修正 (alpha.21、Rancha 提案の 2 案を両方)**: ①全 7 パレットのキー集合を統一 (欠けを CSS フォールバックの解決値と同じリテラルで補完 = 見た目不変、「全パレット同一キー集合」をテストで固定) ②`applyTheme` は要素の inline `--ric-*` を自分の管理下として、新パレットに無いものを `removeProperty` (FACT: 残したい値は `createTheme(base, overrides)` で渡す。非 `--ric-` は触らない)。**教訓: alpha.18 の「全テーマに `none` を明示」は正しい規律だったが、テストで固定していなかったので既存の 3 キーが漏れたまま残っていた**
- **実害 B (ricdom、別件): スクロールバー装飾が Chromium 121+ で 1 行も描画に使われていない**。同じ要素に `scrollbar-width: thin` / `scrollbar-color` (標準) と `::-webkit-scrollbar*` (8px・角丸) を両方当てており、Chromium 121+ は標準プロパティが `auto` 以外なら webkit 疑似要素をまるごと無視する。Rancha が device px で実測 (thumb 13 device px = thin の幅、角丸なし。`auto` に戻すと 12 device px = 8 CSS px + 角丸)。`getComputedStyle(el, '::-webkit-scrollbar-thumb')` は解決値を返すだけで描画に使われるかは返さない → テストで見抜けない。**v0.4.2 以来、Chromium 121+ の全 consumer は「thin の標準バー」を見ていた**。**ユーザー決定: 標準プロパティ 1 本に倒す (webkit ブロック削除)** — Chromium 121+ / Firefox では見た目が今と完全に同じ (変化ゼロ)、8px 角丸の復活案は全 Chromium consumer の見た目を変えるため不採用。古い Electron (<28) / Safari <18.2 はネイティブに戻る。`--ric-scrollbar-thumb-hover` は標準に相当が無いのでトークンは残し「未使用」と SPEC に明記。v1 は保守モードのため据え置き (同じ CSS を持つが、直すなら v0.4.7)
- **Electron 側の問題 (docs のみ)**: frameless + `backgroundMaterial` は**本物の最大化** (`win.maximize()` / Win+↑ / タイトルバー dblclick / 上端ドラッグ) で全面真っ黒になり acrylic は二度と復帰しない (`frame: true` では起きない = Chromium のカスタムフレーム実装の問題)。材質の掛け直し・bounds nudge・hide/show・minimize/restore・fullscreen 往復の 12 手すべて無効。**唯一の対策 = glass の間だけ `maximizable: false` + `setBounds(workArea)` の擬似最大化** (半分スナップ・最小化・ドラッグ・角丸・影は残る、擬似最大化中は bounds を保存しない)。TUTORIAL §6 に採録。他: 派生トークンは `:root` ではなく `applyTheme` した要素かその子孫に書く (inline 変数は祖先からしか継承されない) / glass-dark が明るい壁紙で沈む件は新トークンを作らず `--ric-color-bg: rgba(15,23,42,0.35)` の薄い膜を選択肢として docs に
- **実装・検証 (alpha.21)**: 統括の実ブラウザ実測で cyber → dark 後に `--ric-popup-bg` = `#111318` (dark の値)、`--ric-popup-blur` = `none`、非 `--ric-` の inline (`--app-x`、`width`) は保持、`data-ricdom-theme` = `dark`。scrollbar は computed `scrollbar-width: thin` + `scrollbar-color` にテーマ色。**エージェントの削除は `[data-ricdom-theme]` スコープだけで、`.ric-scroll-pane` にも同じ dead code (標準プロパティ + webkit 併記) が残っていた → 統括が同じ alpha で削除、テストは「stylesheet 全体に `::-webkit-scrollbar` が 1 つも無い」に強化** (エージェントは「スコープ外」として残す判断をしていた = 委任の範囲指定が狭かった)。unit 721 / browser 全 green / examples 8。ui IIFE 26,272 → 26,237B (キー統一 +98B、webkit 削除 −133B)、ui.css 6,973 → 6,848B、コア不変
- 第 6 号は完了、回避策は alpha.21 でゼロ。テーマ 7 種対応は TG / Rancha の 2 アプリで報告済み

## 38. テーマ 7 種対応の第 1 報 = Trend Guard #16「`.ric-panel` が `--ric-color-bg` を見ていて透過レシピで面が消える」→ `--ric-panel-bg` (2026-09-17、2.0.0-alpha.20)

- **報告**: 依頼と同日に 7 種すべて切替可 (⚙ 設定のラジオ)、既存 5 テーマ不変、Electron 透過あり (Win11 / Electron 42 / DPI 150%、`backgroundMaterial: 'acrylic'`)、unit 148 / E2E 11、公開用スクリーンショット 2 枚 (合成 4 銘柄、アイコン列除去)。アプリ側 CSS の変更は回避策 1 箇所のみ — ヘッダ・フッター・判定色セルは元から `rgba()` / `color-mix(…, transparent)` だったので無変更で可読
- **#16 (実害確定)**: TUTORIAL §6 の透過レシピは `--ric-color-bg` を `transparent` にするが、`.ric-panel` の `background` が同じトークンを見ていたため**全パネルの面が一緒に消える** (computed `rgba(0,0,0,0)`、blur だけ残る)。glass は明るい壁紙なら偶然それらしく見えるが、**glass-dark は fg がほぼ白のまま面が暗くならず、明るい壁紙の上で文字が読めない** (最初のスクリーンショットがその状態)。§36 で「透明ウィンドウでもアプリが自分の背景の面倒を見る」と書いた一方で、ricdom 自身の面がページ背景トークンに依存していた = 自分の原則を自分が破っていた
- **修正**: **パネル専用トークン `--ric-panel-bg`** (TG 提案の 2 案目)。既存 5 テーマは値を `--ric-color-bg` と同一にして見た目不変 (同値をテストで固定)、glass `rgba(255,255,255,0.45)` / glass-dark `rgba(15,23,42,0.5)`、reduced-transparency の override にも追加。`.ric-panel { background: var(--ric-panel-bg, var(--ric-color-bg)) }` で自前パレットは従来どおり。**1 案目 (`--ric-color-control` を見る) を採らなかった理由: light の panel `#f9fafb` と control `#ffffff` が違い、既存テーマの見た目が変わるため**
- **原則を FACT 化 (SPEC §8)**: 浮遊面・容器面は面のトークン (`--ric-color-control` / `--ric-popup-bg` / `--ric-panel-bg`) を見る、ページ背景トークン `--ric-color-bg` は見ない。透過レシピが消すのはページ背景だけで、面は残る。`${bg}` の他の使用箇所は棚卸しして「ページ的 (正しい) / 面 (直す)」を判定 → **結果 (9 箇所)**: 面として直したのは `.ric-panel` と **`.ric-tweak`** (tweak パネルも `backdrop-filter` を持つ浮遊面なのに `${bg}` を見ていた = 同じ穴、alpha.18 の時点で漏れていた) の 2 つ。ページ的で正しいもの: `[data-ricdom-theme]` の塗り (ページそのもの)、`.ric-splitter__collapse-btn` (周囲に溶ける意図)、`.ric-accordion__header` / `__body-inner`、`.ric-tabs--pill .ric-tabs__bar`、`.ric-tweak-folder__header` (いずれもフロー内の構造的な塗りで浮遊面ではない)。要検討 1: `.ric-tweak-row__json` (tweak 内の装飾ボックス、透過レシピでは tweak の面の上に乗るので実害なし、次に tweak の glass 表示で報告があれば `--ric-panel-bg` 系に寄せる)。浮遊面 8 種の面トークンの対応表: dialog / toast = `--ric-popup-bg` (fallback bg)、popup / dropdown / inline-menu = `--ric-color-control`、tooltip = `--ric-tooltip-bg`、panel / tweak = `--ric-panel-bg` (SPEC §8 FACT)。統括の実ブラウザ実測 (透過レシピ + glass-dark + `uiPanel`): root `rgba(0,0,0,0)`、panel `rgba(15,23,42,0.5)` + blur、fg `#f1f5f9` が明るい壁紙の上で可読。ui IIFE 26,204 → 26,272B (+68B)、コア不変。unit 712 / browser 158 / examples 8
- **TUTORIAL §6 に `backgroundColor: '#00000000'`**: TG が `backgroundColor: '#ffffff'` を指定していて acrylic が完全に塞がれ glass が真っ白になった。既存アプリほど踏む穴 (新規アプリはそもそも指定しない)。TG 提案の回帰テスト「透過レシピ適用後もパネルの computed background の alpha > 0」を browser に追加 (修正前に赤を確認)
- 脚注として依頼文に追加: DPI 150% では `BrowserWindow.getBounds()` (論理 px) と OS キャプチャ (物理 px) の変換を忘れるとウィンドウが切れる (ricdom 無関係、他 Electron consumer の撮影向け)。第 2 号は 18 件、回避策は alpha.20 でゼロに戻る
- **追報 (同日、alpha.20 取り込み) で #16 クローズ・回避策撤去**: 回避策 CSS 無しの同条件で alpha.19 / alpha.20 の vendor を差し替えて 7 テーマのパネル computed `background-color` を実測 → light / dark / cyber / aqua は前後同値、**glass `rgba(0,0,0,0)` → `rgba(255,255,255,0.45)`、glass-dark → `rgba(15,23,42,0.5)`**。「既存 5 テーマは 1px も変わらない」を consumer 実機でも裏取り。unit 148 / E2E 11。TG の所感: パネル専用トークンを選んだ判断 (control に寄せると light の panel `#f9fafb` と control `#ffffff` がずれる) は提案時に見落としていた、実測で正しさを確認
- **テスト設計の注意 (TG 指摘)**: teal のように `--ric-panel-bg` が `linear-gradient(...)` だけのテーマは `background-color` が透明で `background-image` 側に入る (cyber / aqua の radial-gradient は末尾に solid 色があるので `background-color` に出る)。**グラデーションのテーマで `background-color` だけを見る assert は意味を持たない** → 面の存在を見るテストは glass / glass-dark に絞るか `background-image` も見る。v2 の browser テストは glass / glass-dark (alpha > 0) と light / dark (solid の同値) に絞っており、この罠を踏んでいない — 今後 teal / aqua に広げるときの前提として記録
